...
while(cap.isOpened()):
    ...
    mask2 = cv2.inRange(hsv, lower_red,upper_red) #mask 2 is everything except the cloak

    mask1=mask1+mask2 

    mask1 = cv2.morphologyEx(mask1, cv2.MORPH_OPEN, np.ones((3,3),np.uint8),iterations=2) #Noise removal
    mask1 = cv2.morphologyEx(mask1, cv2.MORPH_DILATE, np.ones((3,3),np.uint8),iterations=1) #Image smoothening
    mask2 = cv2.bitwise_not(mask1)

    res1 = cv2.bitwise_and(background, background ,mask=mask1) #Segmentation of color
    res2 = cv2.bitwise_and(img,img, mask=mask2) # Substituting the cloak part
